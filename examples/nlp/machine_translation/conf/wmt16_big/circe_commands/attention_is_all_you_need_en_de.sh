pip install -r requirements/requirements.txt \
  ...
  && cat ${bi_path}/train.clean.en ${bi_path}/train.clean.de ${mono_en_path}/monolingual.2* ${mono_de_path}/monolingual.2* > all_text.txt \
  && yttm bpe --data all_text.txt --model bpe_16k_en_de_yttm.model --vocab_size 16000 \
  && python train.py -cn wmt16_big/en_de_8gpu \
      trainer.gpus=16 \
      ...
      exp_manager.exp_dir=/workspace/result_en_de \
      trainer.max_epochs=50 \
  && python test.py -cn wmt16_big/en_de_8gpu \
      trainer.gpus=16 \
      model.train_ds.src_file_name=${bi_path}/train.clean.en \
      model.train_ds.tgt_file_name=${bi_path}/train.clean.de \
      model.validation_ds.src_file_name=${bi_path}/wmt13-en-de.src \
